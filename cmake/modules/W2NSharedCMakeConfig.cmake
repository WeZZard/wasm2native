...
macro(w2n_common_standalone_build_config_llvm product)
  ...
  # This could be computed using ${CMAKE_CFG_INTDIR} if we want to link w2n
  # against a matching LLVM build configuration.  However, we usually want to be
  # flexible and allow linking a debug w2n against optimized LLVM.
    ...
    if(LLVM_TABLEGEN)
      ...
      if(CMAKE_CROSSCOMPILING)
        ...
        if(NOT EXISTS "${LLVM_NATIVE_BUILD_DIR}")
          message(FATAL_ERROR
            "Attempting to cross-compile w2n standalone but no native LLVM build
  ...
  # Work around a bug in the w2n-driver that causes the w2n-driver to not be
  # able to accept .tbd files when linking without passing in the .tbd file with
  # a -Xlinker flag.
  #
  # Both clang and w2nc can accept an -Xlinker flag so we use that to pass the
  # value.
  if (APPLE)
    ...
    foreach(lib ${LLVMSUPPORT_INTERFACE_LINK_LIBRARIES})
      ...
      # value. So we need to work around this until we get a newer w2nc that
  ...
  include(AddLLVM)
  include(AddW2NTableGen) # This imports TableGen from LLVM.
  ...
  # however, due to the custom scheme w2n uses for cross-compilation, the 
  ...
  # add_w2n_host_library and add_w2n_target_library within Addw2n.cmake.
  ...
  set(w2n_LIBCLANG_LIBRARY_VERSION
...
# Parameters:
#   product
#     The product name, e.g. w2n or SourceKit. Used as prefix for some
#     cmake variables.

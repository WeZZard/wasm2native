...
#ifndef W2N_BASIC_EXTERNALUNION_H
#define W2N_BASIC_EXTERNALUNION_H

#include <llvm/Support/ErrorHandling.h>
#include <cassert>
#include <cstdint>
#include <utility>
#include <w2n/Basic/type_traits.h>
...
struct ExternalUnionMembers {
  ...
  static constexpr Index indexOf() {
    static_assert(
      ExternalUnionImpl::indexOf<T, Members...>::value != -1,
  ...
  /// of the list.  If it is not -1, it may be safely converted to an
  /// Index.
  template <class T>
...
public:
  ...
  T& emplace(Index index, Args&&... args) {
    ...
    static_assert(typeIndex != -1, "type not in union");
    assert(
      index == Index(typeIndex) && "current kind is wrong for value"
    );
  ...
  T& get(Index index) {
    ...
    static_assert(typeIndex != -1, "type not in union");
    assert(
      index == Index(typeIndex) && "current kind is wrong for access"
    );
  ...
  void resetToEmpty(Index oldIndex, Index newIndex) {
    ...
    assert(
      oldIndex == Index(typeIndex) && "current kind is wrong for value"
    );
    assert(
      newIndex == Index(voidTypeIndex) && "new kind is not in union"
    );
  ...
  void copyAssign(
    Index thisIndex, Index otherIndex, const BasicExternalUnion& other
  ) {
    if (this == &other) {
      // do nothing
    } else if (thisIndex == otherIndex) {
      Members::Info::copyAssignSame(
        unsigned(thisIndex), Storage, other.Storage
      );
    } else {
      destruct(thisIndex);
  ...
  void moveAssign(
    Index thisIndex, Index otherIndex, BasicExternalUnion&& other
  ) {
    assert(this != &other && "move-constructing value into itself?");

    if (thisIndex == otherIndex) {
      Members::Info::moveAssignSame(
        unsigned(thisIndex), Storage, other.Storage
      );
    } else {
      destruct(thisIndex);
...
public:
  ...
  enum : bool {
    union_is_trivially_copyable =
      decltype(Union)::union_is_trivially_copyable
  };
...
#ifndef NDEBUG
    return Union.template emplace<T>(
      GetIndexForKind(kind), std::forward<Args>(args)...
    );
#else
...
#ifndef NDEBUG
    return Union.template emplaceAggregate<T>(
      GetIndexForKind(kind), std::forward<Args>(args)...
    );
#else
...
#ifndef NDEBUG
    return Union.template resetToEmpty<T>(
      GetIndexForKind(curKind), GetIndexForKind(newKind)
    );
#else
  ...
  /// Copy-assign the union from another union.
  void
  copyAssign(Kind thisKind, Kind otherKind, const ExternalUnion& other) {
    Union.copyAssign(
      GetIndexForKind(thisKind), GetIndexForKind(otherKind), other.Union
    );
  }
  ...
  void moveAssign(Kind thisKind, Kind otherKind, ExternalUnion&& other) {
    Union.moveAssign(
      GetIndexForKind(thisKind),
      GetIndexForKind(otherKind),
...
class SimpleExternalUnionBase :
  public ExternalUnion<
    typename KindHelper::Kind,
    Members,
...
public:
  ...
  T * dyn_cast(Kind kind) {
    return (
      kind == kindForMember<T>() ? &this->template get<T>(kind) : nullptr
    );
  }
...
public:
  ...
  enum : bool {
    ...
    is_copy_constructible =
      Member::is_copy_constructible && Others::is_copy_constructible,
    is_nothrow_copy_constructible = Member::is_nothrow_copy_constructible
                                 && Others::is_nothrow_copy_constructible,
    is_move_constructible =
      Member::is_move_constructible && Others::is_move_constructible,
    is_nothrow_move_constructible = Member::is_nothrow_move_constructible
                                 && Others::is_nothrow_move_constructible,
    is_copy_assignable =
      Member::is_copy_assignable && Others::is_copy_assignable,
    is_nothrow_copy_assignable = Member::is_nothrow_copy_assignable
                              && Others::is_nothrow_copy_assignable,
    is_move_assignable =
      Member::is_move_assignable && Others::is_move_assignable,
    is_nothrow_move_assignable = Member::is_nothrow_move_assignable
                              && Others::is_nothrow_move_assignable,
  };

  enum : size_t {
    size = Member::size > Others::size ? Member::size : Others::size,
    alignment = Member::alignment > Others::alignment ? Member::alignment
                                                      : Others::alignment
  };

  LLVM_ATTRIBUTE_ALWAYS_INLINE
  static void
  copyConstruct(void * self, unsigned index, const void * other) {
    if (index == 0) {
  ...
  LLVM_ATTRIBUTE_ALWAYS_INLINE
  static void
  copyAssignSame(unsigned index, void * self, const void * other) {
    if (index == 0) {
...
#endif // W2N_BASIC_EXTERNALUNION_H

...
/// enforce purity in the w2n codebase.
class SourceLoc {
...
template <> struct DenseMapInfo<w2n::SourceLoc> {
  ...
  static bool isEqual(const w2n::SourceLoc &LHS,
                      const w2n::SourceLoc &RHS) {
    return LHS == RHS;
  }
};

template <> struct DenseMapInfo<w2n::SourceRange> {
  ...
  static bool isEqual(const w2n::SourceRange &LHS,
                      const w2n::SourceRange &RHS) {
    return LHS == RHS;
  }
};
} // namespace llvm

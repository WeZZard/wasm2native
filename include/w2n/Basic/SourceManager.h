...
public:
  ...
  StringRef
  extractText(CharSourceRange Range, Optional<unsigned> BufferID = None) const;
  ...
  resolveFromLineCol(unsigned BufferId, unsigned Line, unsigned Col) const;
  ...
  getLocForLineCol(unsigned BufferId, unsigned Line, unsigned Col) const {
    ...
    return Offset.has_value() ? getLocForOffset(BufferId, Offset.getValue())
                             : SourceLoc();
  }

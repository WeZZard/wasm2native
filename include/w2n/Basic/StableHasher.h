...
#ifndef W2N_BASIC_STABLEHASHER_H
#define W2N_BASIC_STABLEHASHER_H
...
/// A \c StableHasher is an implementation of a 128-bit stable hash -
/// built for speed.
///
/// A "stable" hash in this context refers to the idea that the output of
/// this hasher is deterministic across instantiations of the compiler. In
/// order to support this goal, this hasher disallows run-dependent or
/// otherwise unstable values from entering into the hash-combiner. For
/// example, this hasher will statically reject attempts to hash-combine
/// pointers and aggregates of pointers. Note that this relies on user
/// cooperation as well. The order of hash-combines is pivotal, thus e.g.
/// collection values should have a guaranteed order or be sorted before
/// being hash-combined.
///
/// Stable hash values must also be independent of the host architecture.
/// For integral types and enumerations, the default hash-combiner will
/// automatically byte-swap to a common little-endian format.
///
/// This hasher also allows for extending the hash-combiner to
/// user-defined types. To do so, define a (partial) specialization of \c
/// w2n::StableHasher::Combiner<T>
///
///    template <typename T>
///    struct w2n::StableHasher::Combiner<std::optional<T>> {
///      static void combine(StableHasher &hasher, const std::optional<T>
///      &O) {
///        if (!O.has_value()) {
///          hasher.combine(0);
///        } else {
///          hasher.combine(1);
///          w2n::StableHasher::Combiner<T>::combine(hasher, O.value());
///        }
///      }
///    };
///
/// The current implementation is the 128-bit (extended) SipHash 2-4,
/// which has been empirically demonstrated to have the best throughput
...
public:
  ...
  void combine(EnumType value) {
    ...
    return this->template combine<Underlying>(
      static_cast<Underlying>(value)
    );
  }

  template <typename T>
  auto combine(const T * ptr)
    -> decltype("Cannot hash-combine pointers!"){};
...
#endif // W2N_BASIC_STABLEHASHER_H

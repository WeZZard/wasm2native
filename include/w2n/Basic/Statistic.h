...
#ifndef W2N_BASIC_STATISTIC_H
#define W2N_BASIC_STATISTIC_H
...
#define W2N_FUNC_STAT W2N_FUNC_STAT_NAMED(DEBUG_TYPE)

#define W2N_FUNC_STAT_NAMED(DEBUG_TYPE)                                  \
  do {                                                                   \
    static llvm::Statistic FStat = {DEBUG_TYPE, __func__, __func__};     \
    ++FStat;                                                             \
  } while (0)

// Helper class designed to consolidate reporting of LLVM statistics and
// timers across swift compilations that typically invoke many drivers,
// each running many frontends. Additionally collects some cheap
// "always-on" statistics, beyond those that are (compile-time)
// parameterized by -DLLVM_ENABLE_STATS (LLVM's stats are global and
// involve some amount of locking and mfences).
//
// Assumes it's given a process name and target name (the latter used as
// decoration for its self-timer), and a directory to collect stats into,
// then:
//
//  - On construction:
...
//
// Generally we make one of these per-process: either early in the life of
// the driver, or early in the life of the frontend.
...
/// Get the number of instructions executed since this process was
/// launched. Returns 0 if the number of instructions executed could not
/// be determined.
...
#undef FRONTEND_STATISTIC
  ...
  // To trace an entity, you have to provide a TraceFormatter for it. This
  // is a separate type since we do not have retroactive conformances in
  // C++, and it is a type that takes void* arguments since we do not have
  // existentials separate from objects in C++. Pity us.
  struct TraceFormatter {
    virtual void
    traceName(const void * Entity, raw_ostream& OS) const = 0;
    virtual void traceLoc(
      const void * Entity,
  ...
  // -trace-stats-events, but we recycle the same FrontendStatsTracers to
  // give us some free recursion-save phase timings whenever
  // -trace-stats-dir is active at all. Reduces redundant machinery.
  class RecursionSafeTimers;

  // We also keep a few banks of optional hierarchical profilers for times
  // and statistics, activated with -profile-stats-events and
  // -profile-stats-entities, which are part way between the detail level
  // of the aggregate statistic JSON files and the fine-grained CSV
  // traces. Naturally these are written in yet a different file format:
  // the input format for flamegraphs.
...
private:
  ...
  // This is unique_ptr because NamedRegionTimer is
  ...
  /// Whether we are currently flushing statistics and should not
  /// therefore record any additional stats until we've finished.
...
public:
  ...
  void
  saveAnyFrontendStatsEvents(FrontendStatsTracer const& T, bool IsEntry);
  void recordJobMaxRSS(long rss);
  int64_t getChildrenMaxResidentSetSize();
};

// This is a non-nested type just to make it less work to write at call
// sites.
class FrontendStatsTracer {
  FrontendStatsTracer(
  ...
  // In the general case we do not know how to format an entity for
  // tracing.
  template <typename T>
  static const UnifiedStatsReporter::TraceFormatter *
  getTraceFormatter() {
    return nullptr;
  }
...
public:
  ...
  /// These are the convenience constructors you want to be calling
  /// throughout the compiler: they select an appropriate trace formatter
  /// for the provided entity type, and produce a tracer that's either
...
// In particular cases, we do know how to format traced entities: we
// declare explicit specializations of getTraceFormatter() here, matching
// the overloaded constructors of FrontendStatsTracer above, where the
// _definitions_ live in the upper-level files (in libswiftAST or
// libswiftSIL), and provide tracing for those entity types. If you want
// to trace those types, it's assumed you're linking with the object files
// that define the tracer.
...
// Provide inline definitions for the delegating constructors.  These
// avoid introducing a circular dependency between libParse and libSIL.
// They are marked as `inline` explicitly to prevent ODR violations due to
// multiple emissions.  We cannot force the inlining by defining them in
// the declaration due to the explicit template specializations of the
// `getTraceFormatter`, which is declared in the `FrontendStatsTracer`
...
template <typename T, typename U>
FrontendStatsTracer make_tracer_pointerunion(
  UnifiedStatsReporter * Reporter,
  StringRef Name,
...
FrontendStatsTracer make_tracer(
  UnifiedStatsReporter * Reporter,
  StringRef Name,

...
#ifndef W2N_BASIC_SUCCESSORMAP_H
#define W2N_BASIC_SUCCESSORMAP_H
...
public:
  ...
  void insert(KeyTy&& key, ValueTy&& value) {
    ...
    Root =
      new Node(std::forward<KeyTy>(key), std::forward<ValueTy>(value));
    Root->Left = lowerBound;
    Root->Right = upperBound;
    if (upperBound)
      upperBound->Left = nullptr;
  }
...
private:
  ...
  bool splay(const K& key) {
    ...
    auto rotateAsLowerRoot = [&](Node * node) {
      ...
      // The left child goes in the rightmost position of the old lower
      // tree. The right child gets dropped, and its position is the new
      // rightmost position.
    ...
    auto reassemble = [&](bool foundUpperBound) {
      ...
      *upperLeftmost = cur->Right;
      ...
      Root = cur;
      assert(
        !foundUpperBound || Root->Left == nullptr
        || Root->Left->Right == nullptr
      );
      return foundUpperBound;
    };

    // A helper to finish the operation, given that 'cur' is an upper
    // bound.
    ...
    while (true) {
      ...
      if (Traits::precedes(key, cur->Begin)) {
        ...
        // Otherwise, check if we should recurse into the left-left
        // subtree.
        ...
        assert(
          !Traits::precedes(key, left->End) && "key already mapped!"
        );
  ...
  static void validateNode(
    Node * node, Optional<K> lowerBound, Optional<K> upperBound
  ) {
...
#endif // W2N_BASIC_SUCCESSORMAP_H

...
void SourceManager::verifyAllBuffers() const {
  ...
  LLVM_ATTRIBUTE_UNUSED static char arbitraryTotal = 0;
...
void SourceManager::createVirtualFile(
  ...
  if (
    CachedVFile.first &&
    Range.contains(SourceLoc(llvm::SMLoc::getFromPointer(CachedVFile.first)))) {
...
bool SourceManager::openVirtualFile(
  ...
  if (
    nextRangeIter != VirtualFiles.end() &&
...
void SourceManager::closeVirtualFile(SourceLoc end) {
  ...
  virtualFile->Range =
    CharSourceRange(*this, virtualFile->Range.getStart(), end);
...
unsigned
SourceManager::getColumnInBuffer(SourceLoc Loc, unsigned BufferID) const {
  assert(Loc.isValid());

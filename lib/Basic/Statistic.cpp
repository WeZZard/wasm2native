...
bool environmentVariableRequestedMaximumDeterminism() {
  if (const char * S = ::getenv("W2NC_MAXIMUM_DETERMINISM"))
    return (S[0] != '\0');
  return false;
}
...
static std::string makeFileName(
  ...
  stream << Prefix << "-" << usec.count() << "-" << ProgramName << "-"
         << AuxName << "-" << Process::GetRandomNumber() << "." << Suffix;
  return stream.str();
}

static std::string
makeStatsFileName(StringRef ProgramName, StringRef AuxName) {
  return makeFileName("stats", ProgramName, AuxName, "json");
}

static std::string
makeTraceFileName(StringRef ProgramName, StringRef AuxName) {
  return makeFileName("trace", ProgramName, AuxName, "csv");
}

static std::string
makeProfileDirName(StringRef ProgramName, StringRef AuxName) {
  return makeFileName("profile", ProgramName, AuxName, "dir");
}

// LLVM's statistics-reporting machinery is sensitive to filenames
// containing YAML-quote-requiring characters, which occur surprisingly
// often in the wild; we only need a recognizable and likely-unique name
// for a target here, not an exact filename, so we go with a crude
// approximation. Furthermore, to avoid parse ambiguities when
// "demangling" counters and filenames we exclude hyphens and slashes.
...
public:
  ...
  void printToFile(StringRef Dirname, StringRef Filename) const {
    ...
    if (EC) {
      llvm::errs() << "Error opening profile file '" << Path
                   << "' for writing\n";
      return;
    }
    print(Stream);
...
void UnifiedStatsReporter::noteCurrentProcessExitStatus(int status) {
...
void UnifiedStatsReporter::publishAlwaysOnStatsToLLVM() {
  if (FrontendCounters) {
...
void UnifiedStatsReporter::printAlwaysOnStatsAndTimers(raw_ostream& OS) {
...
void updateProcessWideFrontendCounters(
  ...
  C.MaxMallocUsage = std::max(
    C.MaxMallocUsage, (int64_t)llvm::sys::Process::GetMallocUsage()
  );
#endif
}

static inline void saveEvent(
  StringRef StatName,
  int64_t Curr,
  int64_t Last,
  uint64_t NowUS,
...
void UnifiedStatsReporter::saveAnyFrontendStatsEvents(
  ...
  // If we don't have a saved entry to form deltas against in the trace
  // buffer or profilers, we're not tracing or profiling: return early.
  ...
  if (EventProfilers) {
    ...
    EventProfilers->UserTime.profileEvent(
      T.EventName, TimeDelta.getUserTime(), IsEntry
    );
    EventProfilers->SystemTime.profileEvent(
      T.EventName, TimeDelta.getSystemTime(), IsEntry
    );
    EventProfilers->ProcessTime.profileEvent(
      T.EventName, TimeDelta.getProcessTime(), IsEntry
    );
    EventProfilers->WallTime.profileEvent(
      T.EventName, TimeDelta.getWallTime(), IsEntry
    );
...
#undef FRONTEND_STATISTIC
  ...
  if (EntityProfilers) {
    ...
    EntityProfilers->UserTime.profileEvent(
    ...
    EntityProfilers->WallTime.profileEvent(
      T.EventName, TimeDelta.getWallTime(), IsEntry, T.Entity, T.Formatter
    );
#define FRONTEND_STATISTIC(TY, N)                                        \
  EntityProfilers->N.profileEvent(                                       \
    T.EventName, Curr.N - Last.N, IsEntry, T.Entity, T.Formatter         \
  );
#include <w2n/Basic/Statistics.def>
#undef FRONTEND_STATISTIC
...
#define FRONTEND_STATISTIC(TY, N)                                        \
  saveEvent(                                                             \
    #TY "." #N, Curr.N, Last.N, NowUS, LiveUS, Events, T, IsEntry        \
  );
#include <w2n/Basic/Statistics.def>
#undef FRONTEND_STATISTIC
  }
...
UnifiedStatsReporter::~UnifiedStatsReporter() {
  ...
  // NB: Timer needs to be Optional<> because it needs to be destructed
  // early; LLVM will complain about double-stopping a timer if you tear
  // down a NamedRegionTimer after printing all timers. The printing
  // routines were designed with more of a global-scope,
  // run-at-process-exit in mind, which we're repurposing a bit here.
  Timer.reset();

  // We currently do this by manual TimeRecord keeping because LLVM has
  // decided not to allow access to the Timers inside NamedRegionTimers.
  ...
  if (FrontendCounters) {
    ...
    if (C.NumSourceLines != 0 && ElapsedTime.getProcessTime() != 0.0)
      C.NumSourceLinesPerSecond = (int64_t
      )(((double)C.NumSourceLines) / ElapsedTime.getProcessTime());
  }
  ...
  // turning on all stats for all of LLVM and clang is a bit more
  // expensive and intrusive than we want to be in release builds.
  //
  //  - If enabled: we copy all of our "always-on" local stats into LLVM's
  //    global statistics list, and ask LLVM to manage the printing of
  //    them.
  //
  //  - If disabled: we still have our "always-on" local stats to write,
  //  and
  //    LLVM's global _timers_ were still enabled (they're
...
void UnifiedStatsReporter::flushTracesAndProfiles() {
  // Note that we're currently flushing statistics and shouldn't record
  // any more until we've finished.
  llvm::SaveAndRestore<bool> flushing(IsFlushingTracesAndProfiles, true);

  if (FrontendStatsEvents && SourceMgr) {
    std::error_code EC;
    raw_fd_ostream tstream(
      TraceFilename, EC, fs::OF_Append | fs::OF_Text
    );
    if (EC) {
    ...
    for (auto const& E : *FrontendStatsEvents) {
      tstream << E.TimeUSec << ',' << E.LiveUSec << ','
              << (E.IsEntry ? "\"entry\"," : "\"exit\",") << '"'
              << E.EventName << '"' << ',' << '"' << E.CounterName << '"'
              << ',' << E.CounterDelta << ',' << E.CounterValue << ',';
      tstream << '"';
      if (E.Formatter)
      ...
      tstream << '"';
      if (E.Formatter)
        E.Formatter->traceLoc(
          E.Entity, SourceMgr, ClangSourceMgr, tstream
        );
      tstream << '"' << '\n';
    }
  }
  ...
  if (EventProfilers || EntityProfilers) {
    std::error_code EC =
      llvm::sys::fs::create_directories(ProfileDirname);
    if (EC) {
...
#define FRONTEND_STATISTIC(TY, NAME)                                     \
  EventProfilers->NAME.printToFile(                                      \
    ProfileDirname, #TY "." #NAME ".events"                              \
  );
#include <w2n/Basic/Statistics.def>
#undef FRONTEND_STATISTIC
    }
    if (EntityProfilers) {
      ...
      EntityProfilers->ProcessTime.printToFile(
        D, "Time.Process.entities"
      );
      EntityProfilers->WallTime.printToFile(D, "Time.Wall.entities");
#define FRONTEND_STATISTIC(TY, NAME)                                     \
  EntityProfilers->NAME.printToFile(                                     \
    ProfileDirname, #TY "." #NAME ".entities"                            \
  );
#include <w2n/Basic/Statistics.def>
#undef FRONTEND_STATISTIC
    }

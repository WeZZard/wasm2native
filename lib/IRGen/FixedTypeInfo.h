//===--- FixedTypeInfo.h - Supplements fixed-layout types -*- C++ -*-===//
//
...
#ifndef W2N_IRGEN_FIXEDTYPEINFO_H
#define W2N_IRGEN_FIXEDTYPEINFO_H
...
private:
  ...
  /// The spare bit mask for this type. SpareBits[0] is the LSB of the
  /// first byte. This may be empty if the type has no spare bits.
...
protected:
  ...
  FixedTypeInfo(
    ...
    TypeInfo(
      type, align, pod, bt, alwaysFixedSize, IsABIAccessible, stik
    ),
    SpareBits(spareBits) {
    assert(SpareBits.size() == size.getValueInBits());
    assert(isFixedSize());
    SizeInBytes = size.getValue();
    assert(SizeInBytes == size.getValue() && "truncation");
  }
...
public:
  ...
  static IsABIAccessible_t isABIAccessible() {
  ...
  StackAddress allocateStack(
    IRGenFunction& IGF, Type * T, const llvm::Twine& name
  ) const override;
  void deallocateStack(IRGenFunction& IGF, StackAddress addr, Type * T)
    const override;
  void destroyStack(
  ...
  llvm::Value * getSize(IRGenFunction& IGF, Type * T) const override;
  llvm::Value *
  getAlignmentMask(IRGenFunction& IGF, Type * T) const override;
  llvm::Value * getStride(IRGenFunction& IGF, Type * T) const override;
  llvm::Value * getIsPOD(IRGenFunction& IGF, Type * T) const override;
  llvm::Value *
  getIsBitwiseTakable(IRGenFunction& IGF, Type * T) const override;
  llvm::Value *
  isDynamicallyPackedInline(IRGenFunction& IGF, Type * T) const override;

  llvm::Constant * getStaticSize(IRGenModule& IGM) const override;
  llvm::Constant * getStaticAlignmentMask(IRGenModule& IGM
  ) const override;
  ...
  /// Returns the known, fixed size required to store a value of this
  /// type.
  ...
  /// The stride is at least one, even for zero-sized types, like the
  /// empty tuple.
  ...
  /// Returns the number of extra inhabitants available by exercising
  /// spare bits.
  unsigned getSpareBitExtraInhabitantCount() const;

  /// We can statically determine the presence of extra inhabitants for
  /// fixed types.
  bool mayHaveExtraInhabitants(IRGenModule& IGM) const override {
    return getFixedExtraInhabitantCount(IGM) > 0;
  }

  /// Get the bit mask that must be applied before testing an extra
  /// inhabitant.
  ...
  virtual APInt getFixedExtraInhabitantValue(
    IRGenModule& IGM, unsigned bits, unsigned index
  ) const {
    return getSpareBitFixedExtraInhabitantValue(IGM, bits, index);
  }

  /// Create an extra inhabitant constant using the spare bits of the
  /// type.
  APInt getSpareBitFixedExtraInhabitantValue(
    IRGenModule& IGM, unsigned bits, unsigned index
  ) const;
  ...
  virtual llvm::Value * getExtraInhabitantIndex(
    IRGenFunction& IGF, Address src, Type * T, bool isOutlined
  ) const {
  ...
  /// index.
  llvm::Value *
  getSpareBitExtraInhabitantIndex(IRGenFunction& IGF, Address src) const;

  /// Store the extra inhabitant representation indexed by a 31-bit
  /// identifier to memory.
  virtual void storeExtraInhabitant(
    IRGenFunction& IGF,
    llvm::Value * index,
    Address dest,
    Type * T,
  ...
  void storeSpareBitExtraInhabitant(
    IRGenFunction& IGF, llvm::Value * index, Address dest
  ) const;

  /// Get the spare bit mask for the type.
  const SpareBitVector& getSpareBits() const {
    return SpareBits;
  }
  ...
  /// Applies the fixed spare bits mask for this type to the given
  /// BitVector, clearing any bits used by valid representations of the
  /// type.
  ///
  /// If the bitvector is empty or smaller than this type, it is grown and
  /// filled with bits direct from the spare bits mask. If the bitvector
  /// is larger than this type, the trailing bits are untouched.
  ///
  /// The intent is that, for all the data types of an enum, you should be
  /// able to do this:
  ///
  ///   SpareBitVector spareBits;
  ...
  void applyFixedSpareBitsMask(
    const IRGenModule& IGM, SpareBitVector& mask
  ) const;

  void collectMetadataForOutlining(
  ...
  llvm::Value * getEnumTagSinglePayload(
    IRGenFunction& IGF,
  ...
  static bool classof(const FixedTypeInfo * type) {
    return true;
  }
...
llvm::Value *
emitLoad1to4Bytes(IRGenFunction& IGF, Address from, llvm::Value * size);
void emitStore1to4Bytes(
  IRGenFunction& IGF, Address to, llvm::Value * val, llvm::Value * size
);

llvm::Value *
emitGetTag(IRGenFunction& IGF, Address from, llvm::Value * size);
void emitSetTag(
  IRGenFunction& IGF, Address to, llvm::Value * val, llvm::Value * size
);
...
#endif // W2N_IRGEN_FIXEDTYPEINFO_H

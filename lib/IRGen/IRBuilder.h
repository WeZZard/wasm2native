...
//===----------------------------------------------------------------===//
//
// This file defines wasm2native's specialization of llvm::IRBuilder.
//
//===----------------------------------------------------------------===//

#ifndef W2N_IRGEN_IRBUILDER_H
#define W2N_IRGEN_IRBUILDER_H
...
public:
  ...
  bool hasPostTerminatorIP() const {
    return GetInsertBlock() != nullptr && !GetInsertBlock()->empty()
  ...
  void SetInsertPoint(
    llvm::BasicBlock * BB, llvm::BasicBlock::iterator before
  ) {
    ClearedIP = nullptr;
  ...
  llvm::Module * getModule() const {
    if (auto BB = GetInsertBlock())
      return BB->getModule();
    assert(
      ClearedIP && "IRBuilder has no active or cleared insertion block"
    );
  ...
  llvm::Value * CreateAnd(
    llvm::Value * LHS, llvm::Value * RHS, const Twine& Name = ""
  ) {
    if (auto * RC = dyn_cast<llvm::Constant>(RHS))
      if (isa<llvm::ConstantInt>(RC) && cast<llvm::ConstantInt>(RC)->isMinusOne())
        return LHS; // LHS & -1 -> LHS
    return IRBuilderBase::CreateAnd(LHS, RHS, Name);
  }

  llvm::Value *
  CreateAnd(llvm::Value * LHS, const APInt& RHS, const Twine& Name = "") {
    return CreateAnd(
      LHS, llvm::ConstantInt::get(LHS->getType(), RHS), Name
    );
  }

  llvm::Value *
  CreateAnd(llvm::Value * LHS, uint64_t RHS, const Twine& Name = "") {
    return CreateAnd(
      LHS, llvm::ConstantInt::get(LHS->getType(), RHS), Name
    );
  }
  ...
  llvm::Value *
  CreateOr(llvm::Value * LHS, llvm::Value * RHS, const Twine& Name = "") {
    if (auto * RC = dyn_cast<llvm::Constant>(RHS))
      if (RC->isNullValue())
  ...
  llvm::Value *
  CreateOr(llvm::Value * LHS, const APInt& RHS, const Twine& Name = "") {
    return CreateOr(
      LHS, llvm::ConstantInt::get(LHS->getType(), RHS), Name
    );
  }
  ...
  llvm::Value *
  CreateOr(llvm::Value * LHS, uint64_t RHS, const Twine& Name = "") {
    return CreateOr(
      LHS, llvm::ConstantInt::get(LHS->getType(), RHS), Name
    );
  }
  ...
  llvm::LoadInst * CreateLoad(
    ...
    llvm::LoadInst * load =
      IRBuilderBase::CreateLoad(elementType, addr, name);
    load->setAlignment(llvm::MaybeAlign(align.getValue()).valueOrOne());
    return load;
  }

  llvm::LoadInst *
  CreateLoad(Address addr, const llvm::Twine& name = "") {
    return CreateLoad(
      addr.getAddress(), addr.getElementType(), addr.getAlignment(), name
    );
  }

  llvm::StoreInst *
  CreateStore(llvm::Value * value, llvm::Value * addr, Alignment align) {
  ...
  // an alignment.
  llvm::LoadInst *
  CreateLoad(llvm::Value * addr, const llvm::Twine& name = "") = delete;
  llvm::StoreInst *
  CreateStore(llvm::Value * value, llvm::Value * addr) = delete;

  using IRBuilderBase::CreateStructGEP;

  Address CreateStructGEP(
    Address address,
    unsigned index,
    Size offset,
    const llvm::Twine& name = ""
    ...
    assert(
      isa<llvm::StructType>(address.getElementType())
      || isa<llvm::ArrayType>(address.getElementType())
    );

    llvm::Value * addr = CreateStructGEP(
      address.getElementType(), address.getAddress(), index, name
    );
    llvm::Type * elementType = nullptr;
    if (auto * structTy = dyn_cast<llvm::StructType>(address.getElementType())) {
      elementType = structTy->getElementType(index);
    } else if (auto * arrTy = dyn_cast<llvm::ArrayType>(address.getElementType())) {
      elementType = arrTy->getElementType();
    }

    return Address(
      addr, elementType, address.getAlignment().alignmentAtOffset(offset)
    );
  }
  ...
  Address CreateConstArrayGEP(
    ...
    auto addr = CreateConstInBoundsGEP1_32(
      base.getElementType(), base.getAddress(), index, name
    );
    return Address(
      addr,
      base.getElementType(),
  ...
  Address CreateConstByteArrayGEP(
    Address base, Size offset, const llvm::Twine& name = ""
  ) {
    auto addr = CreateConstInBoundsGEP1_32(
  ...
  Address CreateElementBitCast(
    Address address, llvm::Type * type, const llvm::Twine& name = ""
  ) {
  ...
  llvm::CallInst * CreateMemCpy(Address dest, Address src, Size size) {
    return CreateMemCpy(
      ...
      src.getAddress(),
      llvm::MaybeAlign(src.getAlignment().getValue()),
      size.getValue()
    );
  }
  ...
  llvm::CallInst *
  CreateMemSet(Address dest, llvm::Value * value, Size size) {
    return CreateMemSet(
      dest.getAddress(),
      value,
      size.getValue(),
  ...
  llvm::CallInst *
  CreateMemSet(Address dest, llvm::Value * value, llvm::Value * size) {
    return CreateMemSet(
      dest.getAddress(),
      value,
      size,
  ...
  llvm::CallInst * CreateCall(
    ...
    assert(
      (!DebugInfo || getCurrentDebugLocation()) && "no debugloc on call"
    );
    assert(!isTrapIntrinsic(Callee) && "Use CreateNonMergeableTrap");
    auto Call =
      IRBuilderBase::CreateCall(FTy, Callee, Args, Name, FPMathTag);
    setCallingConvUsingCallee(Call);
    return Call;
  }

  llvm::CallInst * CreateCallWithoutDbgLoc(
    llvm::FunctionType * FTy,
    llvm::Constant * Callee,
    ArrayRef<llvm::Value *> Args,
    const Twine& Name = "",
  ...
  llvm::CallInst *
  CreateCall(const FunctionPointer& fn, ArrayRef<llvm::Value *> args);
  ...
  llvm::CallInst * CreateAsmCall(
    llvm::InlineAsm * asmBlock, ArrayRef<llvm::Value *> args
  ) {
  ...
  llvm::CallInst * CreateIntrinsicCall(
    ...
    return CreateCallWithoutDbgLoc(
      cast<llvm::FunctionType>(intrinsicFn->getValueType()),
      intrinsicFn,
      args,
      name
    );
  ...
  llvm::CallInst * CreateExpect(
    llvm::Value * value, llvm::Value * expected, const Twine& name = ""
  ) {
    return CreateIntrinsicCall(
      llvm::Intrinsic::expect, {value->getType()}, {value, expected}, name
    );
  }

  /// Call the trap intrinsic. If optimizations are enabled, an inline asm
  /// gadget is emitted before the trap. The gadget inhibits transforms
  /// which merge trap calls together, which makes debugging crashes
  /// easier.
  llvm::CallInst *
  CreateNonMergeableTrap(IRGenModule& IGM, StringRef failureMsg);
  ...
  std::array<llvm::Value *, N> CreateSplit(llvm::Value * aggregate) {
    ...
    assert(
      cast<llvm::StructType>(aggregate->getType())->getNumElements() == N
    );
  ...
  llvm::Value * CreateCombine(
    llvm::StructType * aggregateType, ArrayRef<llvm::Value *> values
  ) {
...
/// Given a Builder as input to its constructor, this class resets the
/// Builder so it has the same insertion point at end of scope.
class IRBuilder::SavedInsertionPointRAII {
  IRBuilder& builder;
  PointerUnion<llvm::Instruction *, llvm::BasicBlock *>
    savedInsertionPoint;

public:

  /// Constructor that saves a Builder's insertion point without changing
  /// the builder's underlying insertion point.
  SavedInsertionPointRAII(IRBuilder& inputBuilder) :
    builder(inputBuilder),
    savedInsertionPoint() {
    // If our builder does not have a valid insertion point, just put
    // nullptr into SavedIP.
    if (!builder.hasValidIP()) {
      savedInsertionPoint = static_cast<llvm::BasicBlock *>(nullptr);
      return;
    }

    // If we are inserting into the end of the block, stash the insertion
    // block.
  ...
  SavedInsertionPointRAII(
    IRBuilder& b, llvm::Instruction * newInsertionPoint
  ) :
  ...
  SavedInsertionPointRAII(
    IRBuilder& b, llvm::BasicBlock * insertionBlock
  ) :
    SavedInsertionPointRAII(b) {
  ...
  SavedInsertionPointRAII&
  operator=(const SavedInsertionPointRAII&) = delete;

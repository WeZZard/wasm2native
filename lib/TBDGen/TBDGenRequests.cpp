...
void w2n::simple_display(
  llvm::raw_ostream& os, const TBDGenDescriptor& ss
) {
  os << "Generate TBD for ";
  if (auto * Module = ss.getFileOrModule().dyn_cast<ModuleDecl *>()) {
    os << "module ";
    simple_display(os, Module);
  } else {
    os << "file ";
    simple_display(os, ss.getFileOrModule().get<FileUnit *>());
  }
}

...
public:
  ...
  void handleDiagnostic(SourceManager& SM, const DiagnosticInfo& Info)
    ...
    EXPECT_EQ(
      Located<StringRef>(Info.FormatString, Info.Loc), expected.front()
    );
...
TEST(FileSpecificDiagnosticConsumer, SubconsumersFinishInOrder) {
  ...
  auto consumerA =
    std::make_unique<ExpectationDiagnosticConsumer>(nullptr, None);
  auto consumerUnaffiliated =
    std::make_unique<ExpectationDiagnosticConsumer>(
      consumerA.get(), None
    );
...
TEST(FileSpecificDiagnosticConsumer, InvalidLocDiagsGoToEveryConsumer) {
  ...
  ExpectedDiagnostic expected[] = {
    Located<StringRef>("dummy", SourceLoc())};
  auto consumerA =
    std::make_unique<ExpectationDiagnosticConsumer>(nullptr, expected);
  auto consumerUnaffiliated =
    std::make_unique<ExpectationDiagnosticConsumer>(
...
TEST(
  ...
  topConsumer->handleDiagnostic(
    sourceMgr,
    testDiagnosticInfo(frontOfA, "front", DiagnosticKind::Error)
  );
  topConsumer->handleDiagnostic(
    sourceMgr,
    testDiagnosticInfo(frontOfB, "front", DiagnosticKind::Error)
  );
  topConsumer->handleDiagnostic(
    sourceMgr,
...
TEST(FileSpecificDiagnosticConsumer, NotesAreAttachedToErrors) {
  ...
  topConsumer->handleDiagnostic(
    sourceMgr,
    testDiagnosticInfo(frontOfA, "error", DiagnosticKind::Error)
  );
  topConsumer->handleDiagnostic(
  ...
  topConsumer->handleDiagnostic(
    sourceMgr,
    testDiagnosticInfo(frontOfB, "error", DiagnosticKind::Error)
  );
  topConsumer->handleDiagnostic(
...
TEST(
  ...
  topConsumer->handleDiagnostic(
    sourceMgr,
    testDiagnosticInfo(SourceLoc(), "note", DiagnosticKind::Note)
  );
  topConsumer->handleDiagnostic(
    sourceMgr,
    testDiagnosticInfo(frontOfB, "error", DiagnosticKind::Error)

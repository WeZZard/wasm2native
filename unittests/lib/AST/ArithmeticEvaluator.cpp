...
namespace w2n {
#define W2N_TYPEID_ZONE   ArithmeticEvaluator
#define W2N_TYPEID_HEADER "ArithmeticEvaluatorTypeIDZone.def"
...
TEST(ArithmeticEvaluator, Simple) {
  ...
  ArithmeticExpr * product = new Binary(
    Binary::OperatorKind::Product, sum, lifeUniverseEverything
  );
  ...
  EXPECT_EQ(
    evalOrNaN(evaluator, InternallyCachedEvaluationRule(product)),
    expectedResult
  );
  ...
  EXPECT_EQ(
    evalOrNaN(evaluator, UncachedEvaluationRule(product)), expectedResult
  );
  EXPECT_EQ(
    evalOrNaN(evaluator, UncachedEvaluationRule(product)), expectedResult
  );
  ...
  evaluator.clearCache();
  EXPECT_EQ(
    evalOrNaN(evaluator, ExternallyCachedEvaluationRule(product)),
    expectedResult
  );
...
TEST(ArithmeticEvaluator, Cycle) {
  ...
  EXPECT_TRUE(
    std::isnan(evalOrNaN(evaluator, UncachedEvaluationRule(product)))
  );
